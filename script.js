...
function initAgentDemo(){
  ...
  btn.addEventListener("click", async () => {
    ...
    status.textContent = "The AI agent is thinking. This may take up to 90 seconds.";
    ...
    const payload = {
      ...
      message: prompt,
      value: prompt,
      user_context: "Website visitor using Flow Strategic AI one-free-test universal AI agent demo.",
      output_format: "Premium practical answer with clear next steps.",
      research_mode: "Use tools when useful.",
    ...
    try{
      ...
      status.textContent = "Free AI test complete. Want this built into your business? Contact Flow Strategic AI below.";
      result.style.display = "block";
      result.textContent = reply || raw || "The agent responded, but no readable text was returned.";

      btn.textContent = "Free Test Used";
      btn.disabled = true;
    }catch(err){
      clearTimeout(timeout);
      console.error("Universal agent demo error:", err);
      status.className = "agent-status bad";
      status.textContent = err.name === "AbortError"
        ? "The agent took too long. Please try again or contact Flow Strategic AI."
        : "The demo could not connect right now. Please contact Flow Strategic AI.";
      btn.disabled = false;
      btn.textContent = "Run Free AI Test";
    }
  });
}
